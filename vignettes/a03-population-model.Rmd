...
Then re-run the population model projections, but this time add on environmental stressors. Note that in this simple example two imaginary stressors are added `CE_df1` and `CE_df2` with fake data. These stressors target fry capacity and juvenile survival, respectively. The first stressor (`My Stressor1`) reduces the fry carrying capacity by 18% (0.82) and the second stressor decreases the fry survival by 20% (0.80).
...
Rerun the population model. Note the plots below the red line is the baseline and the black line is the population projection run with stressors included. In each example the population is suppressed when the stressors are included.
